...
#define ElfFileParams                                                          \
  class Elf_Ehdr, class Elf_Phdr, class Elf_Shdr, class Elf_Addr,              \
      class Elf_Off, class Elf_Dyn, class Elf_Sym, class Elf_Versym,           \
      class Elf_Verdef, class Elf_Verdaux, class Elf_Verneed,                  \
      class Elf_Vernaux, class Elf_Rel, class Elf_Rela, unsigned ElfClass
#define ElfFileParamNames                                                      \
  Elf_Ehdr, Elf_Phdr, Elf_Shdr, Elf_Addr, Elf_Off, Elf_Dyn, Elf_Sym,           \
      Elf_Versym, Elf_Verdef, Elf_Verdaux, Elf_Verneed, Elf_Vernaux, Elf_Rel,  \
      Elf_Rela, ElfClass
...
private:
  void checkRange(std::size_t i) {
    if (i >= size())
      throw std::out_of_range("error: Span access out of range.");
  }
...
private:
  ...
  getSectionIndex(const SectionName &sectionName) const;
...
private:
  ...
  void changeRelocTableSymIds(const Elf_Shdr &shdr, RemapFn &&old2newSymId) {
    static_assert(std::is_same_v<ElfRelType, Elf_Rel> ||
                  std::is_same_v<ElfRelType, Elf_Rela>);
  ...
  template <class T, class U> auto follow(U *ptr, size_t offset) -> T * {
  ...
  /* Convert an integer in big or little endian representation (as
     ...
     representation. */
  template <class I> constexpr I rdi(I i) const noexcept;

  /* Convert back to the ELF representation. */
  template <class I, class U> constexpr inline I wri(I &t, U i) const {
