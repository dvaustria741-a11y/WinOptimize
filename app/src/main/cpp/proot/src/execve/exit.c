...
static int bind_proc_pid_auxv(const Tracee *ptracee) {
  ...
  if (binding != NULL &&
      compare_paths(binding->guest.path, guest_path) == PATHS_ARE_EQUAL) {
  ...
  binding =
      insort_binding3(ptracee, ptracee->life_context, host_path, guest_path);
  if (binding == NULL)
    return -1;
...
static int transfer_load_script(Tracee *tracee) {
  ...
  needs_executable_stack =
      (tracee->load_info->needs_executable_stack ||
  ...
  padding_size =
      (stack_pointer - string1_size - string2_size) % STACK_ALIGNMENT;
  ...
  script_size =
      ...
      (needs_executable_stack ? LOAD_STATEMENT_SIZE(*statement, make_stack_exec)
                              : 0) +
...
void translate_execve_exit(Tracee *tracee) {
  ...
  if (status < 0)
    note(tracee, ERROR, INTERNAL, "can't transfer load script: %s",
         strerror(-status));
