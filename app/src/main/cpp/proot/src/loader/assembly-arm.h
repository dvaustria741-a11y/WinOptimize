...
#define BRANCH(stack_pointer, destination)                                     \
  do {                                                                         \
    asm volatile("// Restore initial stack pointer.	\n\t"                      \
                 "mov sp, %0				\n\t"                                          \
                 "					\n\t"                                                   \
                 "// Clear rtld_fini.			\n\t"                                  \
                 "mov r0, #0				\n\t"                                          \
...
#define PREPARE_ARGS_1(arg1_) register word_t arg1 asm("r0") = arg1_;
...
#define OUTPUT_CONTRAINTS_6 OUTPUT_CONTRAINTS_3, "r"(arg4), "r"(arg5), "r"(arg6)
...
#define SYSCALL(number_, nb_args, args...)                                     \
  ({                                                                           \
    ...
    PREPARE_ARGS_##nb_args(args) asm volatile(                                 \
        "push {r7}		\n\t"                                                      \
        "mov r7, #" STRINGIFY_EXPANDED(                                        \
            number_) "\n\t"                                                    \
                     "svc #0x00000000	\n\t"                                    \
                     "pop {r7}		\n\t" : "=r"(                          \
                         result) : OUTPUT_CONTRAINTS_##nb_args : "memory");    \
    result;                                                                    \
  })
#else
#define SYSCALL(number_, nb_args, args...)                                     \
  ({                                                                           \
    ...
    PREPARE_ARGS_##nb_args(args) asm volatile(                                 \
        "svc #0x00000000	\n\t" : "=r"(result) : "r"(number),            \
        OUTPUT_CONTRAINTS_##nb_args : "memory");                               \
    result;                                                                    \
  })
#endif
