...
static inline void clear(word_t start, word_t end) {
  ...
  start_aligned =
      (word_t *)(start_bytes ? start + sizeof(word_t) - start_bytes : start);
...
void _start(void *cursor) {
  ...
  while (1) {
    ...
    case LOAD_ACTION_MMAP_ANON:
      status =
          SYSCALL(MMAP, 6, stmt->mmap.addr, stmt->mmap.length, stmt->mmap.prot,
    ...
    case LOAD_ACTION_MAKE_STACK_EXEC:
      SYSCALL(MPROTECT, 3, stmt->make_stack_exec.start, 1,
    ...
    case LOAD_ACTION_START: {
      ...
      if (unlikely(traced))
        SYSCALL(EXECVE, 6, 1, stmt->start.stack_pointer,
