...
#undef REG
#define REG(tracee, version, index)                                            \
  (*(word_t *)(tracee->is_aarch32 ? (((uint8_t *)&tracee->_regs[version]) +    \
                                     reg_offset_armeabi[index])                \
                                  : (((uint8_t *)&tracee->_regs[version]) +    \
                                     reg_offset[index])))
...
void print_current_regs(Tracee *tracee, int verbose_level,
  ...
  note(tracee, INFO, INTERNAL,
       "pid %d: %s: %s(0x%lx, 0x%lx, 0x%lx, 0x%lx, 0x%lx, 0x%lx) = 0x%lx "
       "[0x%lx, %d]",
       tracee->pid, message, stringify_sysnum(get_sysnum(tracee, CURRENT)),
       ...
       peek_reg(tracee, CURRENT, STACK_POINTER), get_abi(tracee));
}
...
#define RESTORE(sysarg)                                                        \
  (void)(reg_offset[SYSARG_RESULT] != reg_offset[sysarg] &&                    \
    ...
    if (including_sysnum &&
        current_sysnum != REG(tracee, ORIGINAL, SYSARG_NUM)) {
 ...
 */
int push_regs(Tracee *tracee) { return push_specific_regs(tracee, true); }
...
word_t get_systrap_size(Tracee *tracee) {
  ...
  if (tracee->is_aarch32 &&
      (((unsigned char *)&tracee->_regs[CURRENT])[0x40] & 0x20) != 0) {
    return 2;
  }
#else
  (void)tracee;
