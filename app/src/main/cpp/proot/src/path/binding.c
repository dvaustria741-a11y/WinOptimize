...
#define HEAD(tracee, side)                                                     \
  (side == GUEST ? (tracee)->fs->bindings.guest                                \
                 : (side == HOST ? (tracee)->fs->bindings.host                 \
                                 : (tracee)->fs->bindings.pending))

#define NEXT(binding, side)                                                    \
  (side == GUEST ? CIRCLEQ_NEXT(binding, link.guest)                           \
                 : (side == HOST ? CIRCLEQ_NEXT(binding, link.host)            \
                                 : CIRCLEQ_NEXT(binding, link.pending)))

#define CIRCLEQ_FOREACH_(tracee, binding, side)                                \
  for (binding = CIRCLEQ_FIRST(HEAD(tracee, side));                            \
       binding != (void *)HEAD(tracee, side); binding = NEXT(binding, side))

#define CIRCLEQ_INSERT_AFTER_(tracee, previous, binding, side)                 \
  do {                                                                         \
    switch (side) {                                                            \
    case GUEST:                                                                \
      CIRCLEQ_INSERT_AFTER(HEAD(tracee, side), previous, binding, link.guest); \
      break;                                                                   \
    case HOST:                                                                 \
      CIRCLEQ_INSERT_AFTER(HEAD(tracee, side), previous, binding, link.host);  \
      break;                                                                   \
    default:                                                                   \
      CIRCLEQ_INSERT_AFTER(HEAD(tracee, side), previous, binding,              \
                           link.pending);                                      \
      break;                                                                   \
    }                                                                          \
...
#define CIRCLEQ_INSERT_BEFORE_(tracee, next, binding, side)                    \
  do {                                                                         \
    switch (side) {                                                            \
    case GUEST:                                                                \
      CIRCLEQ_INSERT_BEFORE(HEAD(tracee, side), next, binding, link.guest);    \
      break;                                                                   \
    case HOST:                                                                 \
      CIRCLEQ_INSERT_BEFORE(HEAD(tracee, side), next, binding, link.host);     \
      break;                                                                   \
    default:                                                                   \
      CIRCLEQ_INSERT_BEFORE(HEAD(tracee, side), next, binding, link.pending);  \
      break;                                                                   \
    }                                                                          \
    (void)talloc_reference(HEAD(tracee, side), binding);                       \
  } while (0)

#define CIRCLEQ_INSERT_HEAD_(tracee, binding, side)                            \
  do {                                                                         \
    switch (side) {                                                            \
    case GUEST:                                                                \
    ...
    case HOST:                                                                 \
      ...
      break;                                                                   \
    default:                                                                   \
      CIRCLEQ_INSERT_HEAD(HEAD(tracee, side), binding, link.pending);          \
      break;                                                                   \
    }                                                                          \
    (void)talloc_reference(HEAD(tracee, side), binding);                       \
  } while (0)
...
static void print_bindings(const Tracee *tracee) {
  ...
  CIRCLEQ_FOREACH_(tracee, binding, GUEST) {
    if (compare_paths(binding->host.path, binding->guest.path) ==
      ...
      note(tracee, INFO, USER, "binding = %s:%s", binding->host.path,
           binding->guest.path);
  }
}
...
Binding *get_binding(const Tracee *tracee, Side side,
  ...
  CIRCLEQ_FOREACH_(tracee, binding, side) {
     ...
     */
    if (side == HOST && compare_paths(root_path, "/") != PATHS_ARE_EQUAL &&
...
#define CIRCLEQ_REMOVE_ALL(name)                                               \
  do {                                                                         \
...
Binding *new_binding(Tracee *tracee, const char *host, const char *guest,
  ...
  if (status < 0) {
    if (must_exist && getenv("PROOT_IGNORE_MISSING_BINDINGS") == NULL)
      note(tracee, WARNING, INTERNAL, "can't sanitize binding \"%s\": %s", host,
           strerror(-status));
    goto error;
  }
  binding->host.length = strlen(binding->host.path);
...
static void initialize_binding(Tracee *tracee, Binding *binding) {
  ...
  if (compare_paths(binding->guest.path, "/") != PATHS_ARE_EQUAL) {
    ...
    tracee->glue_type =
        (status < 0 || S_ISBLK(statl.st_mode) || S_ISCHR(statl.st_mode)
             ? S_IFREG
             : statl.st_mode & S_IFMT);
    ...
    if (status < 0) {
      note(tracee, WARNING, INTERNAL,
           "sanitizing the guest path (binding) \"%s\": %s", path,
           strerror(-status));
      return;
    }
...
static void add_induced_bindings(Tracee *tracee, const Binding *new_binding) {
  ...
  CIRCLEQ_FOREACH_(tracee->reconf.tracee, old_binding, GUEST) {
    ...
    join_paths(path2, new_binding->guest.path,
    ...
    VERBOSE(tracee, 2,
            ...
            old_binding->host.path, old_binding->guest.path, path,
            new_binding->guest.path, induced_binding->host.path,
