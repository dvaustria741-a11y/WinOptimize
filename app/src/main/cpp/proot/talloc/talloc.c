...
static inline void _tc_set_name_const(struct talloc_chunk *tc,
                                      const char *name);
static struct talloc_chunk *_vasprintf_tc(const void *t, const char *fmt,
                                          va_list ap);
...
static inline void _talloc_chunk_set_not_free(struct talloc_chunk *tc) {
  ...
  tc->flags =
      talloc_magic | ((tc->flags & TALLOC_FLAG_MASK) & ~TALLOC_FLAG_FREE);
}
...
_PUBLIC_ void *_talloc_pooled_object(const void *ctx, size_t type_size,
...
_talloc_total_limit_size(const void *ptr, struct talloc_memlimit *old_limit,
...
static void talloc_abort_type_mismatch(const char *location, const char *name,
                                       const char *expected) {
  const char *reason;
...
static inline size_t
_talloc_total_mem_internal(const void *ptr, enum talloc_mem_count_type type,
  ...
  for (c = tc->child; c; c = c->next) {
    total += _talloc_total_mem_internal(TC_PTR_FROM_CHUNK(c), type, old_limit,
                                        new_limit);
  }
...
static void talloc_report_depth_FILE_helper(const void *ptr, int depth,
  ...
  if (tc->limit && tc->limit->parent == tc) {
    fprintf(f,
            ...
            depth * 4, "", name, (unsigned long)tc->limit->max_size,
