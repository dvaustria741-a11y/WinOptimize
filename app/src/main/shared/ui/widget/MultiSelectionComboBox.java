...
public class MultiSelectionComboBox extends AppCompatTextView {
  ...
  public void setItems(String[] items, String text) {
    this.items = items;
    this.text = text;
    if (!text.isEmpty()) setText(items.length + " " + text);
  ...
  public void setSelectedItems(String[] selectedItems) {
    ...
    if (!text.isEmpty()) setText(selectedItemSet.size() + " " + text);
    else setText(getSelectedItemsAsString());
  }
  ...
  public String getSelectedItemsAsString() {
    ...
    for (String item : items)
      if (selectedItemSet.contains(item)) result += (!result.isEmpty() ? "," : "") + item;
    return result;
  }
  ...
  public boolean performClick() {
    ...
    final ArrayAdapter<String> adapter =
        new ArrayAdapter<String>(getContext(), R.layout.multi_select_list_item, items) {
          @NonNull @Override
          public View getView(int position, @Nullable View convertView, @NonNull ViewGroup parent) {
            CheckedTextView checkedTextView =
                (CheckedTextView) super.getView(position, convertView, parent);
    ...
    popupWindow.setBackgroundDrawable(
        ContextCompat.getDrawable(getContext(), R.drawable.content_popup_menu_background));
